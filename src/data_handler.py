...
class DataCollatorForMatching:
    ...
    def __call__(self, samples: List[List[List[List[int]]]]) -> Dict[str, torch.Tensor]:
        ...
        for i, sample in (enumerate(samples)):
            ...
            input_ids_key_and_neighbors, attention_mask_key_and_neighbors, mask_key_and_neighbors = self.create_training_sample(sample)
        ...
        if self.mlm:
            input_ids_query_and_neighbors_batch, mlm_labels_query_batch = self.mask_tokens(self._tensorize_batch(input_ids_query_and_neighbors_batch, self.tokenizer.pad_token_id), self.tokenizer.mask_token_id)
            input_ids_key_and_neighbors_batch, mlm_labels_key_batch = self.mask_tokens(self._tensorize_batch(input_ids_key_and_neighbors_batch, self.tokenizer.pad_token_id), self.tokenizer.mask_token_id)
        else:
            input_ids_query_and_neighbors_batch = self._tensorize_batch(input_ids_query_and_neighbors_batch, self.tokenizer.pad_token_id)
            input_ids_key_and_neighbors_batch = self._tensorize_batch(input_ids_key_and_neighbors_batch, self.tokenizer.pad_token_id)
    ...
    def _tensorize_batch(self, sequences: Union[List[torch.Tensor], List[List[torch.Tensor]]], padding_value) -> torch.Tensor:
    ...
    def create_training_sample(self, sample: List[List[List[int]]]):
        ...
        def process_node_and_neighbors(tokens_node_and_neighbors):
            ...
            for i,tokens in enumerate(tokens_node_and_neighbors):
                ...
                input_ids_node_and_neighbors.append(torch.tensor(self.tokenizer.build_inputs_with_special_tokens(tokens[:max_num_tokens])))
        ...
        input_ids_query_and_neighbors, attention_mask_query_and_neighbors, mask_query_and_neighbors=process_node_and_neighbors(tokens_query_and_neighbors)
        input_ids_key_and_neighbors, attention_mask_key_and_neighbors, mask_key_and_neighbors=process_node_and_neighbors(tokens_key_and_neighbors)
